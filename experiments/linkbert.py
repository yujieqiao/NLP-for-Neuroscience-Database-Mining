...
with shelve.open("yujie-linkbert") as embeddings:
    for pmid, paper in tqdm.tqdm(pubmed_papers.items()):
        if pmid not in embeddings:
            data = [paper["ArticleTitle"] + get_abstract(paper)]
            inputs = tokenizer(
...
#print(unique_list)
#print(len(unique_list)) 3889


with open("filtering.json") as kiwi:
    unique_list = json.load(kiwi)

# unique_list = [243504, 243506, 245160, 253622]
# #print(unique_list)
#print(len(unique_list)) 451
...
with shelve.open("yujie-451-linkbert-base-matrices") as meta_matrices:
...
#link each "object_id" to its assocaited "object_name". Key is 'object_id', value is 'object_name'
...
f1_nan={}
with shelve.open("yujie-451-linkbert-base-matrices") as meta_matrices:    
    for key, value in tqdm.tqdm(meta_matrices.items()):
        accuracy1=value[0][0]/(value[0][0]+value[0][1])
        accuracy2=value[0][0]/(value[0][0]+value[1][0])
        f1_score=2*accuracy1*accuracy2/(accuracy1+accuracy2)

        if f1_score>0.2:
            f1_high[linkage[int(key)]]=f1_score
        elif f1_score>0.13:
            f1_low[linkage[int(key)]]=f1_score
        else:
            f1_nan[linkage[int(key)]]=f1_score

# pprint(sorted(f1_high.items(), key=lambda x: x[1], reverse=True))
# pprint(sorted(f1_low.items(), key=lambda x: x[1], reverse=True))
# pprint(f1_nan)

with open("f1_linkbert_high.json", "w") as pear:
    pear.write(json.dumps(f1_high))

with open("f1_linkbert_low.json", "w") as apple:
    apple.write(json.dumps(f1_low))
