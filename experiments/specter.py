...
for train_index, test_index in tqdm.tqdm(loo.split(X_all)):
    ...
    # adaboo = AdaBoostClassifier(n_estimators=100, random_state=0)
    # logireg = LogisticRegression(random_state=0, max_iter=5)
...
for train_index, test_index in tqdm.tqdm(loo.split(X_all)):
    ...
    # print("Number of mislabeled points out of a total %d points : %d" % (X_test.shape[0], (y_test != y_pred).sum()))
    if y_test==1 and y_pred==1:
        confusion_mtx2[0][0]+=1
    elif y_test==1 and y_pred==0:
        confusion_mtx2[1][0]+=1
    elif y_test==0 and y_pred==1:
        confusion_mtx2[0][1]+=1
        # with shelve.open("yujie-specter") as embeddings:
        #     for key, value in embeddings.items():
        #         if (row1[int(test_index)]==value).all()==True:
        #             print(key)
...
sns.heatmap(pd.DataFrame(confusion_mtx), annot=True, fmt='g', cmap='YlGnBu', alpha=0.8, vmin=0)
plt.title("Confusion matrix for the tag 'Parkinson's' using SPECTER with GNB", fontsize=10)     

ax=plt.subplot(1,2,2)
sns.heatmap(pd.DataFrame(confusion_mtx2), annot=True, fmt='g', cmap='YlGnBu', alpha=0.8, vmin=0)
plt.title("Confusion matrix for the tag 'Parkinson's' using SPECTER with SVM", fontsize=10)
plt.show()  
